...
/*	使用VG_(details_*)()函数初始化'detail'结构
	使用VG_(basic_tool_funcs)()注册基本tool函数
	以下可选：
	使用VG_(needs_*)()函数初始化'needs'结构
	其它tool专有的初始化
*/
...
/*	插桩函数传递给LibVEX_Translate作为回调函数。
	VgCallbackClosure包含一些信息，插桩函数可能需要。
*/
typedef 
   struct {
...
extern void VG_(basic_tool_funcs)(
   ...
   // 插桩要保证透明性，就是不影响基本块在插桩前后的作用
   
   // VgCallBackClosure* closure包含需要传递给插桩工具的额外参数
   // * closure->tid是需要translation的线程的id，可能callgrind要用
   ...
   //	closure->nraddr是translation的起始地址
   // * closure->readdr is the redirected guest address, from which
   //   the translation was really made.
   //
   //  假设使用Memcheck，当调用malloc()时，客户程序打算调用libc中的malloc()，
   //  此处->nraddr为原malloc()地址；Memcheck替换函数为coregrind/m_replacemalloc/vg_replacemalloc，
   //  于是->readdr为那个函数的地址。
   //
   ...
   //	大多数函数没有被替换，所以->nraddr = ->readdr
   //
   // IRSB* sb_in是即将被插桩的超级块
   //
   ...
   // Memcheck uses this.
   // VexGuestLayout* layout 包含guest state的部分信息
   //
   ...
   // VexGuestExtents* vge 指向一个结构体，指明了被翻译的原代码的地址范围。closure->readdr=vge->base[0]
   //
   ...
   //
   // IRType gWordTy和hWordTy包含了模拟CPU和真实CPU的字长，Ity_I32或Ity_I64
   //
   /* --- Further comments about the IR that your --- */
   /* --- instrumentation function will receive. --- */
   /*
	  对于接收的IRSB，每条指令的IR都从IRStmt_IMark开始
	  ...
	  有时候在IRStmt_IMark之前可能还会有preamble，JIT会用
...
/* Average size of a translation, in bytes, so that the translation
   ...
   setting is optional. */
// 一次翻译的平均字节数，为了翻译存储机制能恰当地分配内存。可选。
...
// 当'tl_assert'断言失败或VG_(tool_panic)被调用时要打印的字符串，通常是email地址
...
// __libc_freeres()是否要运行。该函数尝试释放所有内存
...
extern void VG_(needs_command_line_options) (
   ...
   // 选项由该函数检查，如果有错则调用VG_(fmsg_bad_option)()
...
/* Events happening in core to track.  To be notified, pass a callback
   ...
   对于new_mem_brk and new_mem_stack_signal，提供的tid指明新内存分配给哪个线程
   ...
   对于new_mem_startup and new_mem_mmap，di_handle被用于获取调试信息
*/
...
/* These ones are called when SP changes.  A tool could track these itself
   ...
   对于_new函数，可以指定with-ECU和without-ECU两种版本之一，如果是with-ECU，需要传递ecu参数
*/
