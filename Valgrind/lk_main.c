...
// 四类插桩
//
// * --basic-counts   : 计算被执行的指令、跳转的次数
// * --detailed-counts: 计算存取次数和算术运算的字节数
// * --trace-mem=yes:   跟踪所有内存访问
...
/* 维持事件的有序表 in
   ...
   helper calls.  自顶向下扫描SB然后将内存事件加到有序表末尾, merging with the most recent
   ...
   appear. */
//应该是说以IR粒度判断访存类型并添加到表中，相同项合并。以SB粒度根据表输出访存类型，并刷新表
